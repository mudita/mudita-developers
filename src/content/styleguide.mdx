...
Folders structure of the whole project should be clear and easy to navigate. A project `src` should contain defined structure:
- `/components` is an obvious directory to store all components. This folder has also its own structure inside:

Keep the component related files in one catalogue for ease of finding the relation and working on a single module.
...
Modules should be placed in a flat structure of directories, which should be similar to how storybook is organised.
[More about Storybook organisation](/stories#stories-structure)

- `/server-functions` is a place to store a source code of all Netlify functions. Remember to keep this file flat and containing only endpoints, because of Netlify specification.
- `/server-functions-tests` tests of stored above functions should be placed here. This is an exception from the base rule. Netlify doesn't allow to keep test files in functions folder.
- `/hooks` - put all hooks here.
- `/templates` - all templates for generated pages should be stored here.
- `/pages-creators` - creators, which use above templates to create dynamic pages.
- `/source-nodes-creators` - creators, which generate Gatsby source nodes
- `/pages` - all static pages
- `/images` - all static images if they are needed
- `/constants` - all constants used in the app
...
If a styled component becomes to be more advanced one (with many props or its props are reuseable), wonder about creating a dedicated interface for it.
