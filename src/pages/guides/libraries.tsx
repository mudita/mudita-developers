...
const Libraries: AppFunctionComponent<Props> = ({
  data: {
    libraries: {
      body: libraries,
  ...
  return (
    <Layout>
      ...
      <PageContribution
        ...
        path={"src/content/libraries.mdx"}
      />
      <GuideLayout>
        <StyleguideTableOfContents items={tocItems} />
        <GuideContent>
          <CodeStyleGuide content={libraries} />
        </GuideContent>
      </GuideLayout>
    </Layout>
  )
...
export const query = graphql`
  {
    libraries: mdx(fileAbsolutePath: { glob: "**/libraries.mdx" }) {
      body
      tableOfContents
    }
    github {
      repository(owner: "Appnroll", name: "appnroll-developers") {
        object(expression: "master") {
          ... on GitHub_Commit {
            history(first: 100, path: "src/content/libraries.mdx") {
              nodes {
                authoredDate
                author {
                  user {
